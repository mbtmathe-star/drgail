...
wrap.innerHTML=cart.map((item,index)=>`<div class="cart-item"><img src="${item.image}" alt="${item.title}"><div><h3>${item.title}</h3><small>R${item.price} × ${item.qty} = R${item.price*item.qty}</small></div><input aria-label="Quantity for ${item.title}" type="number" min="1" value="${item.qty}" data-qty="${index}"><button class="remove-item" aria-label="Remove ${item.title}" data-remove="${index}">×</button></div>`).join('');
...
summary.innerHTML=`<p><strong>${cart.reduce((s,i)=>s+i.qty,0)}</strong> item(s)</p><p class="cart-subtotal">Subtotal: <strong>R${subtotal}</strong> <span>(excludes delivery)</span></p>${hasCombo?`<p class="cart-combo-note">Buying the Book + Journal together? The combo price is <strong>R${COMBO_PRICE}</strong> (excludes delivery) — GGM Coaching will adjust your total.</p>`:''}<div class="cart-delivery"><strong>📦 Delivery (via PAXI):</strong><br>R59.95 (7–9 business days)<br>R109.95 (3–5 business days)<br><strong>🚚 Kimberley, Northern Cape:</strong> Free collection or R170 local delivery</div><p class="form-note">Final pricing, delivery fee and payment method will be confirmed directly by GGM Coaching before payment.</p>`;
}
